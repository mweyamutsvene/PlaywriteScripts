...
// Self-contained â€” serialized by Playwright via page.evaluate, no closures.
...
export function collectPaneMessages() {
  ...
  const messageEls = _qa(MESSAGE_SELECTORS, container);
  ...
  messageEls.forEach((el, idx) => {
    ...
    const parsedTime = _parseTimestamp(timeStr);
    if (parsedTime) { lastTime = parsedTime; lastTimeStr = timeStr; }

    let sender = '';
    const senderEl = _q(SENDER_SELECTORS, item);
    ...
    let text = '';
    const bodyEl = _q(MESSAGE_TEXT_SELECTORS, el);
  ...
  const scroller = _getScrollableContainer();
  return {
...
export function scrollPaneUpBy(delta) {
  const el = _getScrollableContainer();
